...
class TestAccount:
    def test_profile_redirect_via_my_profile_button(self, driver, actions):
        actions.redirect_to_profile_page(driver)
        assert WebDriverWait(driver, 5).until(
            ec.visibility_of_element_located(Locators.PROFILE_BUTTON_MY_PROFILE_PAGE)
        )

    def test_constructor_redirect_from_profile_page(self, driver, actions):
        actions.redirect_to_profile_page(driver)
    ...
    def test_logo_redirect_from_profile_page(self, driver, actions):
        actions.redirect_to_profile_page(driver)
